...
def check_structure() -> None:
    ...
    required = [
        ...
        "src/clustering_models.py", "scripts/download_data.py", "scripts/run_all.py",
        "scripts/validate_project.py", "notebooks/regression.ipynb",
        "notebooks/classification.ipynb", "notebooks/clustering.ipynb",
        ...
        "docs/review2_outline.md", "docs/experiment_log.md",
    ]
...
def check_data() -> None:
    ...
    cprep = dl.prepare("clustering", verbose=False)
    bad = [c for c in (config.CLUSTERING["holdout_label"], config.CLUSTERING["id_column"])
           if c in cprep.columns]
    nb = [c for c in cprep.columns if c.startswith("Naive_Bayes_Classifier")]
    check("clustering: Attrition_Flag and CLIENTNUM withheld from fitting frame",
          PASS if not bad else FAIL)
    check("clustering: Naive_Bayes_Classifier columns excluded",
          PASS if not nb else FAIL)
...
def check_model_counts() -> None:
    ...
    b = clf_m.build_part_b(config.CLASSIFICATION["numeric_features"],
                           config.CLASSIFICATION["categorical_features"])
    check("classification: 5 Part-A + 5 Part-B", PASS if len(a) == 5 and len(b) == 5 else FAIL,
          f"A={len(a)} B={len(b)}")
    ...
    base = b["B9. Bagging (Decision Tree base)"].named_steps["model"].estimator
    from sklearn.tree import DecisionTreeClassifier
    check("classification: Bagging base estimator is a Decision Tree",
          PASS if isinstance(base, DecisionTreeClassifier) else FAIL)
...
def check_pipeline_hygiene() -> None:
    ...
    models = {**reg_m.build_models(config.REGRESSION["numeric_features"],
                                   config.REGRESSION["categorical_features"]),
              **clf_m.build_all(config.CLASSIFICATION["numeric_features"],
                                config.CLASSIFICATION["categorical_features"])}
...
def check_metrics_recompute() -> None:
    ...
    c = config.TABLES_DIR / "classification_all10_comparison.csv"
    if c.exists():
        ct = pd.read_csv(c)
        check("classification: 10 rows in consolidated table",
              PASS if len(ct) == 10 else FAIL, f"{len(ct)} rows")
        ...
        check("classification consolidated table", FAIL, "not found")
...
def check_figures() -> None:
    ...
    required = {
        ...
        "classification": ["clf_feature_distributions", "clf_correlation_heatmap",
                           ...
                           "clf_confusion_partA", "clf_confusion_all10",
                           "clf_roc_curves", "clf_decision_tree", "clf_feature_importance"],
        "clustering": ["clu_feature_distributions", "clu_kmeans_elbow",
                       "clu_dendrogram_ward", "clu_pca_kmeans",
                       "clu_pca_agglomerative", "clu_tsne_kmeans"],
    }
...
def check_team_sections() -> None:
    ...
    check("clustering: feature subset justification",
          TODO if "PROVISIONAL" in config.CLUSTERING["feature_subset_status"] else PASS,
          config.CLUSTERING["feature_subset_status"])

    import nbformat
    for nb_name in ("regression", "classification", "clustering"):
...
def check_honesty() -> None:
    ...
    p = config.TABLES_DIR / "clustering_stability.csv"
    if p.exists():
        t = pd.read_csv(p)
        labelled = (t["interpretation_status"]
                    .str.contains("SUPPLEMENTARY").all())
        check("clustering stability labelled as NOT cross-validation",
              PASS if labelled else FAIL)
    p = config.TABLES_DIR / "clustering_final_metrics.csv"
    if p.exists():
        t = pd.read_csv(p)
        check("clustering silhouette exactness recorded", PASS,
              f"exact={bool(t['silhouette_exact'].all())}")


def main() -> int:
    print("=" * 78)
    print("PROJECT VALIDATION - reporting what exists, not what was planned")
    print("=" * 78)
    check_structure()
    check_data()
    check_model_counts()
