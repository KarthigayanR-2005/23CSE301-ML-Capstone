...
mode that would cost marks or make a number wrong:
  ...
  2. the excluded-column contract (Failure Type / Attrition_Flag)
  3. model counts and the exact algorithms the PDF names
...
@needs_data
def test_attrition_flag_never_reaches_the_clustering_frame():
    prep = dl.prepare("clustering", verbose=False)
    assert "Attrition_Flag" not in prep.columns
    assert "CLIENTNUM" not in prep.columns
    assert not [c for c in prep.columns if c.startswith("Naive_Bayes_Classifier")]
    assert prep.shape[1] == 7
...
def test_algorithm_counts_and_identities():
    ...
    b = clf_m.build_part_b(["Torque [Nm]"], [])
    assert len(a) == 5 and len(b) == 5
    # the PDF names Gaussian NB specifically
    assert isinstance(a["A3. Gaussian Naive Bayes"].named_steps["model"], GaussianNB)
    # bagging must use a decision tree base estimator
    bag = b["B9. Bagging (Decision Tree base)"].named_steps["model"]
    assert isinstance(bag.estimator, DecisionTreeClassifier)
...
# ---------------------------------------------------------------- clustering
def test_stability_is_labelled_as_not_cross_validation():
    rng = np.random.default_rng(0)
    X = np.vstack([rng.normal(0, 1, (150, 3)), rng.normal(6, 1, (150, 3))])
    out = clu_m.stability_assessment(X, k=2, algorithm="kmeans", n_repeats=3)
    assert "SUPPLEMENTARY" in out["interpretation_status"]
    assert out["mean_ARI"] > 0.8         # two well-separated blobs are stable


def test_clustering_metrics_flag_approximation():
    rng = np.random.default_rng(0)
    X = np.vstack([rng.normal(0, 1, (300, 3)), rng.normal(6, 1, (300, 3))])
    labels = np.r_[np.zeros(300), np.ones(300)].astype(int)
    exact = ev.clustering_metrics(X, labels)
    approx = ev.clustering_metrics(X, labels, sample_size=100)
    assert exact["silhouette_exact"] is True
    assert approx["silhouette_exact"] is False
    assert exact["Davies_Bouldin"] < 1.0
