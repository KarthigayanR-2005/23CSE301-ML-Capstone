...
from sklearn.metrics import (
    accuracy_score, calinski_harabasz_score, confusion_matrix,
...
# Clustering
# --------------------------------------------------------------------------
def clustering_metrics(X, labels, sample_size: int | None = None,
                       random_state: int = config.RANDOM_STATE) -> dict:
    """Silhouette / Davies-Bouldin / Calinski-Harabasz.

    `sample_size` is passed only to silhouette_score (the O(n^2) one). When it
    is used, the returned dict says so explicitly so no approximation is ever
    reported as exact.
    """
    uniq = len(set(labels))
    if uniq < 2:
        return {"n_clusters": uniq, "Silhouette": np.nan,
                "Davies_Bouldin": np.nan, "Calinski_Harabasz": np.nan,
                "silhouette_exact": False, "note": "degenerate: <2 clusters"}
    ...
    return {
        ...
        "silhouette_exact": sample_size is None,
        "silhouette_sample_size": sample_size if sample_size else "full dataset",
    }
