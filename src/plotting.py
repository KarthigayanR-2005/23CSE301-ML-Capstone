...
# Clustering
# --------------------------------------------------------------------------
def elbow_plot(ks, inertias, silhouettes, name: str, subdir: str = ""):
    fig, ax1 = plt.subplots(figsize=(7.5, 4.6))
    c = sns.color_palette(config.PALETTE)
    l1, = ax1.plot(ks, inertias, "o-", color=c[0], label="Inertia (WCSS)")
    ax1.set_xlabel("Number of clusters (k)")
    ax1.set_ylabel("Inertia (within-cluster sum of squares)")
    ax2 = ax1.twinx()
    l2, = ax2.plot(ks, silhouettes, "s--", color=c[3], label="Silhouette score")
    ax2.set_ylabel("Silhouette score")
    ax2.grid(False)
    ax1.set_title("K-Means elbow curve with silhouette overlay")
...
def dendrogram_plot(X, method: str, name: str, subdir: str = "",
                    truncate_p: int = 30, sample_size: int | None = None,
                    random_state: int = config.RANDOM_STATE):
    """Dendrogram from a real fitted hierarchy.

    If `sample_size` is given, the title says so explicitly - a sampled
    dendrogram is never presented as the full hierarchy.
    """
    Xa = np.asarray(X)
    note = ""
    if sample_size and sample_size < len(Xa):
        rng = np.random.default_rng(random_state)
        idx = rng.choice(len(Xa), size=sample_size, replace=False)
        Xa = Xa[idx]
        note = f" [SAMPLED: {sample_size} of {len(X)} rows]"
    Z = linkage(Xa, method=method)
    fig, ax = plt.subplots(figsize=(12, 5.5))
    dendrogram(Z, truncate_mode="lastp", p=truncate_p, show_leaf_counts=True,
               leaf_rotation=90., leaf_font_size=8., ax=ax)
    ax.set_title(f"Hierarchical dendrogram - {method} linkage "
                 f"(display truncated to last {truncate_p} merges){note}")
    ax.set_xlabel("Cluster size at truncated leaf")
    ax.set_ylabel("Merge distance")
    fig.tight_layout()
    return savefig(fig, name, subdir)


def cluster_scatter_2d(coords, labels, title: str, name: str, subdir: str = "",
                       xlabel: str = "Component 1", ylabel: str = "Component 2",
                       note: str = ""):
    coords = np.asarray(coords)
    fig, ax = plt.subplots(figsize=(7, 5.6))
    pal = sns.color_palette(config.CMAP_QUALITATIVE, n_colors=max(len(set(labels)), 3))
    for i, lab in enumerate(sorted(set(labels))):
        m = np.asarray(labels) == lab
        ax.scatter(coords[m, 0], coords[m, 1], s=9, alpha=0.55,
                   color=pal[i % len(pal)], label=f"Cluster {lab}")
    ax.set_xlabel(xlabel)
    ax.set_ylabel(ylabel)
    ax.set_title(title + (f"\n{note}" if note else ""))
    ax.legend(title="Cluster", fontsize=8, markerscale=2)
    fig.tight_layout()
    return savefig(fig, name, subdir)


def metric_vs_k(df: pd.DataFrame, metric: str, name: str, subdir: str = "",
                hue: str = "algorithm"):
    fig, ax = plt.subplots(figsize=(7.5, 4.4))
    sns.lineplot(data=df, x="k", y=metric, hue=hue, marker="o", ax=ax,
                 palette=config.PALETTE)
