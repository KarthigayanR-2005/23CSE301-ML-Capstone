"""Loading and auditing of the three raw datasets.
...
def load_raw(track: str, verbose: bool = True) -> pd.DataFrame:
    ...
    # BankChurners ships two columns that the dataset author explicitly tells
    # users to drop; remove them here so they never reach an audit table.
    banned = spec.get("banned_columns_prefix")
    if banned:
        drop = [c for c in df.columns if c.startswith(banned)]
        if drop and verbose:
            print(f"[load_raw] dropping author-deprecated columns: {drop}")
        # NOTE: the *expected raw shape* is recorded BEFORE this drop.
        df.attrs["dropped_deprecated"] = drop
...
def prepare(track: str, df: pd.DataFrame | None = None,
    ...
    if track == "clustering":
        keep = list(spec["cluster_features"])
    else:
...
def holdout_labels(df_raw: pd.DataFrame) -> pd.Series:
    """Attrition_Flag, for POST-HOC clustering validation only."""
    col = config.CLUSTERING["holdout_label"]
